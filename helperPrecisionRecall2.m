function PRTable = helperPrecisionRecall2(confmat)
...
precisionNSR = confmat(2,2)/sum(confmat(:,2))*100 ;
recallARR = confmat(1,1)/sum(confmat(1,:))*100;
%recallCHF = confmat(2,2)/sum(confmat(2,:))*100;
recallNSR = confmat(2,2)/sum(confmat(2,:))*100;
